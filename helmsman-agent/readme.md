...
- **Re-enter helmsman every session** — HARD STOP + Gate A when the pack is present ([`helmsman-agent.md`](helmsman-agent.md))
...
3. Agents read **[`helmsman-agent/helmsman-agent.md`](helmsman-agent.md)** for the full workflow (gates, plans, tasks, code). **Every session:** re-read this file and run Gate A when the pack is present.
...
Paths below are **inside `{pack}`** (`helmsman-agent/` when installed in an app). See [`helmsman-agent.md`](helmsman-agent.md) §0.
