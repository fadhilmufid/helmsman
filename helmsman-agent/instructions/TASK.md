...
Related: [`RULES.md`](RULES.md), [`PLAN.md`](PLAN.md), [`../HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md), [`OVERVIEW.md`](OVERVIEW.md), [`INFRASTRUCTURE.md`](INFRASTRUCTURE.md), [`HISTORY.md`](HISTORY.md), [`DOCUMENT.md`](DOCUMENT.md), [`CODE.md`](CODE.md), [`DESIGN.md`](DESIGN.md), [`GREENFIELD.md`](GREENFIELD.md), [`BROWNFIELD.md`](BROWNFIELD.md).

## 1. Purpose

A TASK is the **standalone exhaustive execution plan** for a single user request (Gate E), derived from the Gate D blueprint. One file per request — where it sits among the doc layers:

| | PLAN (`project/plans/`) | TASK (`project/tasks/`) | DOCUMENT | HISTORY |
|---|------|------|----------|---------|
| **What** | Blueprint: platforms, phases, E2E matrix | Application map + exhaustive file-level steps | Feature specs | Change log |
| **When** | Gate D | Gate E | Gate C | After Gate F |
| **Shape** | One file per request | **One standalone file**, no step ceiling | Folder per feature | One file per event |

- Spend Gate E drafting **maximum detail** — 50, 100, or 200+ steps is expected for a large bootstrap; more steps is better.
- Stay `Status: planning` until the Application map (§1.4a) and all steps are written; go `in_progress` only after Gates A–D and the §1.6 gates.
- Steps are **change-oriented** — every step names files/paths and describes the change, not vague goals.
- **Ask when blocked** — one batch with a **Recommended:** default per [`../HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md) §3.
...
1. [ ] **platforms/web/package.json** — Next.js 16 app manifest
   ...
   - How to do it:
     ...
     3. Pin versions to match project/PROJECT-INFRASTRUCTURE.md
...
List paths in scope from `project/PROJECT-INFRASTRUCTURE.md` (e.g. `platforms/api/` greenfield, or `backend/` brownfield).

## 1.6 Execution gates (hard STOP)

Mirror [`HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md) §1.5 and [`RULES.md`](RULES.md) §2.
...
## 1.7 Step granularity

**One step = one deliverable** — a single file, migration, route, page, component, or compose service. Never a folder wildcard (`.../**`, "all API routes", "Drive UI components"). Each DB model/migration, API route, page, and shared component gets its own step with Plan/Spec/Code refs, **How to do it**, and **Step checklist** per §1.5. Greenfield bootstrap lists **every** planned file/route/page/component/platform slug as its own step in this one task file.

**Bad:** `6. [ ] platforms/kardus/src/app/api/** — all API routes`

**Good:** one step per route file, e.g. `6. [ ] platforms/kardus/src/app/api/files/route.ts — GET list, POST upload`, fully expanded per the §1.5 template.

Group steps by phase inside the single file: Clarify → Docs → Design → **Service platforms** → Apps → Wire compose → **E2E verify**.

## 1.8 Exhaustive and standalone

**One `project/tasks/{timestamp}_{slug}.md` covers the full scope of the request — never split because the list is long.** Step count is not a reason to split; add more steps to the same file. No `parent`/`child`/`-phase-` task files, no "implement later" placeholders. Over-specify during `Status: planning`: derive every step from the plan platform inventory + `project/documents/` + `project/design/`, and make every row in **Files expected to change** map to an implementation step (justify any merges in Decisions). Update the in-progress file instead of duplicating.
...
- Application source (paths from `project/PROJECT-INFRASTRUCTURE.md`)
...
- **Scope:** {apps/paths from project/PROJECT-INFRASTRUCTURE.md} | docker | deploy | docs | document | task | multi
- **Paths in scope:** `{path}/`, ... (from `project/PROJECT-INFRASTRUCTURE.md`)

## Context read

Gate A per [`../HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md) §1.4 — list every file read:

- `{pack}/HELMSMAN-AGENT.md`, `{root}/AGENTS.md`
- `instructions/RULES.md`, `instructions/README.md`
- Active mode guide (`instructions/GREENFIELD.md` or `instructions/BROWNFIELD.md`)
- `instructions/OVERVIEW.md`, `instructions/INFRASTRUCTURE.md`, `instructions/DOCUMENT.md`, `instructions/DESIGN.md`, `instructions/PLAN.md`, `instructions/TASK.md`, `instructions/HISTORY.md`
...
- `project/` scan — {config, plans, tasks, histories, documents, design in scope}
...
|----------|-------------|-------------|-------------|
| {what is missing} | {default choice} | {user reply} | project/PROJECT-OVERVIEW.md, etc. |
...
2. [ ] **deploy/platforms/** — deploy cycle (greenfield bootstrap)
   ...
   - How to do it:
     1. Build every platform image per project/PROJECT-INFRASTRUCTURE.md
...
Use **paths and app slugs** from `project/PROJECT-INFRASTRUCTURE.md`. Add `docker`, `deploy`, `docs`, `document`, `task`, or `multi` as needed.
...
5. Deliverables (minimum):
   ...
   - project/PROJECT-INFRASTRUCTURE.md
   - project/PROJECT-AGENTS.md (verify dev/lint/test commands)
   - project/PROJECT-DESIGN.md + project/design/ (from existing UI or N/A)
...
**Do:** name exact paths with before/after per step; derive every step from `project/plans/`, `project/documents/`, `project/design/`, and `CODE.md`; write the Application map (§1.4a) before steps; put Plan/Spec/Code refs + How to do it + Step checklist on every step (§1.5); include explicit E2E verify steps in the final phase; re-open the active file and run the Agent execution checklist each work block while `in_progress` (§1.9); write task files locally (gitignored except `project/tasks/README.md`).

**Don't:** skip the task file for non-trivial work; use vague steps or forbidden shorthand (§1.4); use folder wildcards or bundle "all routes/components" in one step (§1.7); create a TASK before the Gate D plan; set `in_progress` before the Application map is complete, Gates A–D pass, and Files-expected-to-change matches steps; split a task by step count or create parent/child files (§1.8); put feature specs or change logs in TASK (use `project/documents/` and `project/histories/`); rewrite completed task files (append instead); mark a step `[x]` without its Step checklist ticked; implement from memory without re-opening the active file (§1.9).

## 7. Agent Checklist

1. Gate D plan exists; Application map complete (flow + route/module table + build sequence, §1.4a)?
2. Standalone exhaustive — full scope in one file; no forbidden shorthand; Files-expected-to-change matches steps 1:1 (§1.4, §1.8)?
3. CODE.md re-read at task start (sections in Context read); Gates A–D passed before `in_progress`?
4. Every app-source step has Plan + Spec + Code refs; CODE §1–2 on every touched file?
5. E2E local + deploy cycles complete (Gate F); active file re-opened each work block (§1.9)?
6. Task completion checklist checked before `Status: complete`; HISTORY links plan + task + E2E + CODE compliance?
