...
**Integration:** Records results after Gate F — link `project/plans/`, `project/tasks/`, specs, and E2E verify outcomes. See [`RULES.md`](RULES.md). Project overview: [`OVERVIEW.md`](OVERVIEW.md) → `project/PROJECT-OVERVIEW.md`.
...
- Application source code (scope per app name in `project/PROJECT-INFRASTRUCTURE.md` — e.g. `web`, `api`, `backend`, or `multi`)
...
Paths for each concern: see [`project/PROJECT-INFRASTRUCTURE.md`](project/PROJECT-INFRASTRUCTURE.md). Brownfield onboarding entries: see [`BROWNFIELD.md`](BROWNFIELD.md) §5.
...
Scope labels (`web`, `api`, `backend`, etc.) reflect app names in `project/PROJECT-INFRASTRUCTURE.md` — not prescriptive folder names.
...
- Don't edit instruction templates (`instructions/*.md`, pack `HELMSMAN-AGENT.md`) when a history entry suffices
