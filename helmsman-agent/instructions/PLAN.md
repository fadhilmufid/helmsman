...
Related: [`../HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md), [`TASK.md`](TASK.md), [`GREENFIELD.md`](GREENFIELD.md), [`DOCUMENT.md`](DOCUMENT.md), [`OVERVIEW.md`](OVERVIEW.md), [`INFRASTRUCTURE.md`](INFRASTRUCTURE.md).
...
- **Scope:** {from project/PROJECT-INFRASTRUCTURE.md}

## Context read

- Specs: project/documents/{feature}/ — {files read}
- Design: project/design/ — {files read or N/A}
- project/PROJECT-INFRASTRUCTURE.md, project/PROJECT-OVERVIEW.md — {read or "drafting"}
...
**Do:** list every `platforms/<slug>` in the inventory; include E2E local + deploy cycles when compose exists; link `project/documents/` and `project/design/` in Spec refs; note "TASK expands this plan" in Task derivation.

**Don't:** skip PLAN for non-trivial work; put file-level steps in PLAN (use TASK); invent platforms not recorded in clarify / `project/PROJECT-INFRASTRUCTURE.md`; mark a plan complete without E2E matrix results in TASK/history.

---

## 7. Agent checklist

1. Gate C passed before drafting; platform inventory complete (service + application)?
2. E2E matrix includes compose up/down and the deploy image cycle (when applicable)?
3. One exhaustive standalone TASK file created from this plan ([`TASK.md`](TASK.md)) with Plan ref on each step?
4. HISTORY links this plan when work completes?
