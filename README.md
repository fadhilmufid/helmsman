...
- **Already have `AGENTS.md`** — add the **For agents** block from that template (pointer to `helmsman/AGENTS.md`); keep your existing rules.
...
```
{root}/
├── AGENTS.md          ← thin pointer (from template)
├── helmsman/          ← cloned pack
│   ├── AGENTS.md      ← full agent entry (do not copy to root)
│   ├── instructions/
│   └── project/
├── platforms/         ← your app (greenfield)
└── deploy/
```
