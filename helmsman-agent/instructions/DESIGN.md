...
**Integration:** Gate C design layer — feeds PLAN and TASK Spec refs. Index in `project/PROJECT-DESIGN.md`; detail in `project/design/`. See [`RULES.md`](RULES.md) §4.

Related: [`PLAN.md`](PLAN.md), [`TASK.md`](TASK.md), [`DOCUMENT.md`](DOCUMENT.md), [`CODE.md`](CODE.md), [`GREENFIELD.md`](GREENFIELD.md), [`BROWNFIELD.md`](BROWNFIELD.md), [`OVERVIEW.md`](OVERVIEW.md), [`INFRASTRUCTURE.md`](INFRASTRUCTURE.md), [`HISTORY.md`](HISTORY.md), [`../HELMSMAN-AGENT.md`](../HELMSMAN-AGENT.md).

General guide for defining a project design system. **Read-only template** — the **index** lives in [`project/PROJECT-DESIGN.md`](project/PROJECT-DESIGN.md); **detailed specs** live in [`project/design/`](project/design/).

**Brownfield:** document existing UI in `project/PROJECT-DESIGN.md` + `project/design/` before imposing new patterns. **Greenfield:** create design files at clarify per [`GREENFIELD.md`](GREENFIELD.md) before UI code.

## 0. Production-grade UI

Per the production bar ([`RULES.md`](RULES.md) §5): ship **polished, finished** screens for in-scope work — not gray boxes, lorem ipsum, or "style later." Every async flow has loading/error/empty/success states; interactive elements are accessible (visible focus, keyboard, touch targets per §10); CRUD flows are complete with confirmation modals ([`CODE.md`](CODE.md) §11); spacing, hover/focus, and responsive breakpoints are consistent. Default to MVP/wireframe UI **only** when the user explicitly asks.

## 1. Folder layout

```
project/
├── PROJECT-DESIGN.md      ← index only (gitignored) — links + quick token cheat sheet
└── design/
...
| API-only / no UI | `project/design/` N/A — note in `project/PROJECT-DESIGN.md` |
...
## 2. project/PROJECT-DESIGN.md (index only)
...
Define in `project/PROJECT-DESIGN.md` (summary) and `project/design/color-palette-and-tokens.md` (detail):
...
**If the user specifies nothing about theme or colors, use the pack default below and record in `project/PROJECT-DESIGN.md`.**
...
Record in `project/PROJECT-DESIGN.md`:
...
**Brownfield:** prefer the library already in the codebase; document it in `project/PROJECT-DESIGN.md`.

#### Workflow

1. Read stack from [`project/PROJECT-INFRASTRUCTURE.md`](project/PROJECT-INFRASTRUCTURE.md) and any library already chosen in [`project/PROJECT-DESIGN.md`](project/PROJECT-DESIGN.md)
...
4. **Install** in the frontend app path per `project/PROJECT-INFRASTRUCTURE.md` and [`CODE.md`](CODE.md) section 10 (install before import)
5. **Theme** components with project tokens from `project/PROJECT-DESIGN.md` — customize, don't reimplement
6. **Document** the library choice in `project/PROJECT-DESIGN.md` and `project/histories/` when first adopted
...
- No suitable library after documented search — note why in `project/PROJECT-DESIGN.md` or history
...
`project/PROJECT-DESIGN.md` index must link to all design files. `project/design/component-library-and-theming.md` must record:

- Chosen component library (or headless + styling approach)
- How project tokens map to library theme variables
- Which shared UI components live in the frontend app (path per `project/PROJECT-INFRASTRUCTURE.md`)
...
- **Responsive strategy (web apps):** follow the strategy recorded in `project/PROJECT-DESIGN.md` (mobile-first, desktop-first, or balanced) — see §10; never desktop-only layouts that break on mobile
...
Project-specific do's and don'ts belong in `project/design/` files. General guidance:

**Do:** apply the default neutral grayscale (light) when the user is silent on theme (§3); keep semantic colors separate from brand accents; use a styled confirmation modal for deletes; record and follow the responsive strategy in `project/PROJECT-DESIGN.md` + `responsive-breakpoints.md`; use a popular component library before hand-rolling primitives, installing it in the frontend app path before importing ([`CODE.md`](CODE.md) §10).

**Don't:** add ad-hoc colors outside the palette; skip hover/focus states; use `window.confirm`/`window.alert`; ship desktop-only layouts, mobile-overflowing widths, or hover-only interactions; rebuild accessible modal/table/select primitives when a maintained library exists; ship unstyled/single-state pages or defer responsive/error states; use MVP/wireframe UI unless the user explicitly asks.

## 10. Responsive Behavior

**Choose and record a responsive strategy** during Gate B clarify / Gate C design — the agent picks the best fit; user preference overrides. **Brownfield:** document existing responsive behavior in `project/PROJECT-DESIGN.md` before changing breakpoints.
...
**Decision order:** (1) user stated preference → (2) `project/PROJECT-OVERVIEW.md` audience/use context → (3) app type from feature docs → (4) brownfield existing patterns → (5) if still ambiguous, **ask once** in clarify batch.

**Record in:** `project/PROJECT-DESIGN.md` (one line: chosen strategy + rationale) and `project/design/responsive-breakpoints.md` (full breakpoint table + layout notes).
...
`project/PROJECT-DESIGN.md` index should include:
...
2. Read [`project/PROJECT-DESIGN.md`](project/PROJECT-DESIGN.md) index and **all** required files in [`project/design/`](project/design/) (**brownfield:** document existing UI first if missing)
...
5. Responsive strategy recorded in `project/PROJECT-DESIGN.md` and `project/design/responsive-breakpoints.md`?
6. If user silent on theme, default neutral grayscale (light) applied and recorded in `project/PROJECT-DESIGN.md`?
